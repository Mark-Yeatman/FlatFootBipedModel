function [gaitnorm,A,B,C,D] = gait_sensitivity_norm(x0, disturbance, indicators, options)
%
...
global flowdata
    ...
    elseif nargin > 3 
        ...
        if contains('g0',names)
            g0 = options.g0;
        else
            g0 = [out_extra.steps{end}.steplength; out_extra.steps{end}.steplength / out_extra.steps{end}.speed;]';
        end
        if contains('g0',period)
            period = options.period;
        else
            period = 1;
        end
    else
        fprintf('Error: must input x0, disturbance, indicators\n')
    end
