...
while abs(step_size) > 0.001 && abs(step_size) < 100
    ...
    flowdata.Parameters.Biped.b = 0.5* ratio;
    flowdata.Parameters.Biped.asvector = [10, 5, 0.5, flowdata.Parameters.Biped.b*ratio]; 
...
for k = 1:length(fnames)
    physical_length_ratio_sweep_results.(fnames{k}) = ...
        [flip(ratio_results_down.(fnames{k})),ratio_results_center.(fnames{k}), ratio_results_up.(fnames{k})];
end
save Experiments\EnergyShapeAndTrack_Compass\Data\LengthRatioSearch physical_length_ratio_sweep_results -append