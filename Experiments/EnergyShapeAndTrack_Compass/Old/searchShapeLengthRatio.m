...
% clear flowdata ratio_results_up ratio_results_down
% path(pathdef)
...
%     flowdata.Parameters.Shape.b = 0.5 * ratio;
%     flowdata.Parameters.Shape.asvector = [10, 5, 0.5, flowdata.Parameters.Shape.b]; 
%     
...
%        ratio_results_up.Parameters{count} = flowdata.Parameters.Shape;
...
while abs(step_size) > 0.001 && ratio > 1e-5
    ...
    flowdata.Parameters.Shape.a = 0.5;
    flowdata.Parameters.Shape.b = 0.5 * ratio;
    flowdata.Parameters.Shape.asvector = [10, 5, 0.5, flowdata.Parameters.Shape.b* ratio]; 
...
%     toc;
    ...
    if stable && done
       ...
       ratio_results_down.Parameters{count} = flowdata.Parameters.Shape;
...
for k = 1:length(fnames)
    shape_length_ratio_sweep_results.(fnames{k}) = ...
        [flip(ratio_results_down.(fnames{k})),ratio_results_center.(fnames{k}), ratio_results_up.(fnames{k})];
end
save Experiments\EnergyShapeAndTrack_Compass\Data\LengthRatioSearch shape_length_ratio_sweep_results -append