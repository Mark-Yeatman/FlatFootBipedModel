...
clear flowdata ratio_results_up ratio_results_down shape_mass_ratio_EPBC_sweep_results
path(pathdef)
...
flowdata.Parameters.KPBC.Eref.SSupp = 1.540567425373938e+02;
...
while abs(step_size) > 0.001 && abs(step_size) < 100
    ...
    flowdata.Parameters.Shape.Mh = 10 * ratio * 5/10;
...
%     toc;
    ...
    if stable && done
       ...
       step_size = -abs(step_size) / c - step_size;
    end
...
for k = 1:length(fnames)
    shape_mass_ratio_EPBC_sweep_results.(fnames{k}) = ...
        [flip(ratio_results_down.(fnames{k})),ratio_results_center.(fnames{k})];%, ratio_results_up.(fnames{k})];
end
save Experiments\EnergyShapeAndTrack_Compass\Data\MassRatioSearch shape_mass_ratio_EPBC_sweep_results -append